...
static s32 decay_lut[16];
static int vbl_lut[32];
static int trilength_lut[128];

/* noise lookups for both modes */
#ifndef REALTIME_NOISE
static s8 noise_long_lut[APU_NOISE_32K];
static s8 noise_short_lut[APU_NOISE_93];
...
INLINE s8 shift_register15(u8 xor_tap)
{
...
#else /* !REALTIME_NOISE */
static void shift_register15(s8 *buf, int count)
{
   static int sreg = 0x4000;
...
static s32 apu_rectangle_##ch(void) \
{ \
   s32 output, total; \
   int num_times; \
\
...
u8 apu_read(u32 address)
{
   ...
   case APU_SMASK:
      ...
      if (apu.dmc.irq_occurred)
         ...
         apu.irqclear_callback();

		apu.frame_irq_occurred = 0;
...
void apu_enable()
{
	sound_setcallback(apu_process);
}

void apu_process(void *buffer, int num_samples)
{
//   static s32 prev_sample = 0;
...
//         s32 next_sample;
		 ...
		 //super hack: uses q's sound emulators from his mapper dlls
		 //TODO: get permission to use these
		 if (apu.ext && (apu.mix_enable & 0x20))
            accum += apu.ext->process(40);
...
//	else
   ...
   /* clear the callbacks */
   temp_apu->irq_callback = set_irq;
   temp_apu->irqclear_callback = clear_irq;
...
int apu_init()
{
	return(apu_create(0,44100,60,16));
}
