...
static INLINE void drawtileline()
{
	...
	for(i=0;i<32;i++) {
		...
		p[3] = ((*tileptr0 >> 4) & 1) | ((*tileptr1 >> 3) & 2);
		p[4] = ((*tileptr0 >> 3) & 1) | ((*tileptr1 >> 2) & 2);
		p[5] = ((*tileptr0 >> 2) & 1) | ((*tileptr1 >> 1) & 2);
		...
		for(j=0;j<8;j++) {
			...
			if(p[j])
				...
				screenptr[j] = nes.ppu.palette[0] | (((CONTROL1 >> 5) & 7) << 8);;
		}
		screenptr += 8;
		tileptr0++;
		tileptr1++;
	...
	switch(LINECYCLES) {
		...
		case 195:	case 203:	case 211:	case 219:	case 227:	case 235:	case 243:	case 251:
			if(CONTROL1 & 0x08) {
				...
				tmp = ((ppu_memread(tmp | 0x2000) >> (((SCROLL & 2) | (((SCROLL >> 5) & 2) << 1)))) & 3) << 2;
