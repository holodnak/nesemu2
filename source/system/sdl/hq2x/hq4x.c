...
void hq4x_32( unsigned char * pIn, unsigned char * pOut, int Xres, int Yres, int srcBpL, int BpL)
{
  ...
  for (j=0; j<Yres; j++)
  {
    ...
    for (i=0; i<Xres; i++)
    {
      ...
      if (i>0)
      {
        w[1] = *((unsigned int*)(pIn + prevline - sizeof(unsigned int))) & 0xFCFCFC;
        w[4] = *((unsigned int*)(pIn - sizeof(unsigned int))) & 0xFCFCFC;
        w[7] = *((unsigned int*)(pIn + nextline - sizeof(unsigned int))) & 0xFCFCFC;
      }
      else
      {
        w[1] = w[2];
      ...
      if (i<Xres-1)
      {
        w[3] = *((unsigned int*)(pIn + prevline + sizeof(unsigned int))) & 0xFCFCFC;
        w[6] = *((unsigned int*)(pIn + sizeof(unsigned int))) & 0xFCFCFC;
        w[9] = *((unsigned int*)(pIn + nextline + sizeof(unsigned int))) & 0xFCFCFC;
      }
      else
      {
        w[3] = w[2];
    ...
    pOut += BpL * 4 - Xres * 4 * sizeof(unsigned int);
    pIn += srcBpL - Xres * sizeof(unsigned int);
  }
}
